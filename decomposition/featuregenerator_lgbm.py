...
class FeatureGeneratorLGBM(BaseEstimator, TransformerMixin):
    def __init__(self):
    ...
    def fit(self, X, y):
        ...
        for i in range(nodes_num):
            self.feature_generators_list.append(lambda df: df[f'{feature_threshold_df.iloc[i][0]}'] <= feature_threshold_df.iloc[i][1])
        return self
